...
const Chatbot: React.FC<{ onNavigate: (v: AppView) => void, onLeadSubmit: (lead: Lead) => void, theme?: 'dark' | 'light' }> = ({ onNavigate, onLeadSubmit, theme = 'dark' }) => {
  ...
  return (
    <div className={`max-w-4xl mx-auto h-[75vh] flex flex-col px-6 py-12 ${isDark ? '' : 'text-gray-900'}`}>
      ...
      <div className="flex-grow overflow-y-auto space-y-8 mb-8 pr-4 custom-scrollbar">
          ...
          <div key={i} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fadeIn`}>
            ...
            <div className={`max-w-[85%] flex gap-5 ${m.sender === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div className="space-y-4">
                <div className={`p-6 rounded-sm text-sm font-light leading-relaxed border ${m.sender === 'bot' ? (isDark ? 'bg-white/[0.03] border-white/5 text-gray-300' : 'bg-gray-100 border-gray-200 text-gray-800') : (isDark ? 'bg-white text-black border-transparent font-medium' : 'bg-white text-black border-gray-200 font-medium')}`}>
                   {m.text}
                </div>
                {m.options && (
                  <div className="flex flex-wrap gap-2">
                    {m.options.map(o => (
                      <button key={o} onClick={() => handleOption(o)} className={`text-[9px] uppercase tracking-[0.2em] border px-4 py-2 transition-all ${isDark ? 'border-white/10 hover:bg-white hover:text-black' : 'border-gray-300 hover:bg-gray-200 hover:text-gray-900'}`}>
                        {o}
                      </button>
                    ))}
                  </div>
        ...
        {showLeadForm && (
          <div className={`p-8 rounded-sm space-y-6 border border-emerald-500/20 animate-fadeIn ml-14 max-w-lg ${isDark ? 'glass' : 'bg-white border-gray-200'}`}>
             ...
             <form onSubmit={submitLead} className="space-y-4">
                <input required type="text" placeholder="Full Name" className={`w-full border p-3 text-xs focus:outline-none ${isDark ? 'bg-black/40 border-white/10' : 'bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-500'}`} value={leadData.name} onChange={e => setLeadData({...leadData, name: e.target.value})} />
                <input required type="tel" placeholder="Phone Number" className={`w-full border p-3 text-xs focus:outline-none ${isDark ? 'bg-black/40 border-white/10' : 'bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-500'}`} value={leadData.phone} onChange={e => setLeadData({...leadData, phone: e.target.value})} />
                <input required type="email" placeholder="Email Address" className={`w-full border p-3 text-xs focus:outline-none ${isDark ? 'bg-black/40 border-white/10' : 'bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-500'}`} value={leadData.email} onChange={e => setLeadData({...leadData, email: e.target.value})} />
                <textarea required placeholder="Briefly describe your bespoke vision..." rows={3} className={`w-full border p-3 text-xs focus:outline-none resize-none ${isDark ? 'bg-black/40 border-white/10' : 'bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-500'}`} value={leadData.details} onChange={e => setLeadData({...leadData, details: e.target.value})} />
      ...
      <div className={`border p-2 flex gap-4 rounded-sm ${isDark ? 'border-white/10 bg-white/[0.02]' : 'border-gray-200 bg-gray-50'}`}>
        <input 
          type="text" 
          placeholder="Type your query..."
          className={`flex-grow bg-transparent border-none px-6 py-4 text-sm font-light focus:outline-none ${isDark ? '' : 'text-gray-900 placeholder:text-gray-500'}`}
