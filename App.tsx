...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-500`}>
      ...
      />
      <main className={`flex-grow pb-12 ${currentView === 'Home' ? 'pt-0' : 'pt-24'}`}>
      ...
      <div className="opacity-0 hover:opacity-100 transition-opacity fixed bottom-0 left-0 p-2 z-[60]">
        <button onClick={() => setCurrentView('JewelerPortal')} className="text-[6px] uppercase tracking-widest text-current opacity-20">Admin CRM</button>
      </div>

      <Footer theme={userState.theme} onNavigate={setCurrentView} />
      <FloatingConcierge onNavigate={setCurrentView} />
    </div>
  );
};
